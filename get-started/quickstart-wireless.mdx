...
<Steps>
  <Step title="Find the device">
    ```sh
    ./sdk/linux/build/ef list --scan-ble
    ```
    Note the MAC address advertised by `Efference M1`.
  </Step>
  <Step title="Control over BLE">
    ```sh
    ./sdk/linux/build/ef --ble AA:BB:CC:DD:EE:FF info
    ```
  ...
  <Step title="Provision WiFi (once)">
    ```sh
    ./sdk/linux/build/ef --ble AA:BB:CC:DD:EE:FF \
      wifi add MyNetwork MyPassphrase US
    ./sdk/linux/build/ef --ble AA:BB:CC:DD:EE:FF wifi status
    ```
    Use the ISO country code for the network's regulatory domain.
  </Step>
