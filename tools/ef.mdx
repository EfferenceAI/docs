...
On a terminal, a long `download` reports progress on a line that redraws in place
and is erased when the transfer ends. Redirected output carries only the final
`saved <path>` line.
