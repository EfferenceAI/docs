...
| `--udp <host[:port]>` | device streams video and IMU to this host over WiFi/UDP; works with USB or `--ble` control (default port 5005) |
...
```sh
ef-cli calibration [--get]         # show the camera + IMU calibration
ef-cli calibration --camera --set <fx> <fy> <cx> <cy> <xi> <alpha> <W> <H> \
                   [--rectify on|off] [--fov-scale <s>]
                                   # set the double-sphere intrinsics (idle only)
ef-cli calibration --imu --mode <raw|calibrated|both>
                                   # how recordings carry the IMU calibration
ef-cli calibration [--camera|--imu] --reset   # restore the factory default
```

Intrinsics are always published as recording metadata. `--rectify on` makes
the device undistort frames itself, so recordings ship rectilinear rather than
raw fisheye; it needs FEC firmware and is off by default. `--fov-scale` sets
the rectified field of view (default `1.0`; below 1 is wider, above 1 zooms).
...
```sh
ef-cli check-update
ef-cli update [url|firmware.eff]       # a local .eff sideloads over the wire
ef-cli abort-update
```
