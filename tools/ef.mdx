...
ef-cli config ae-settle off   # turn the recording head delay off (persisted)
```
`ef-cli info` and `ef-cli config` both show an `ae-settle` row, and `ef-cli info
--json` carries it as `ae_settle`. See
[Recording head delay](/recording/overview#recording-head-delay).
