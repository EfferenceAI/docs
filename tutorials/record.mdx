...
<Tabs>
  <Tab title="Wired">
    ...
    int main() {
        ...
        // 15 s, not a few: with the recording head delay on (the default) the
        // device holds the opening frames until auto-exposure converges, which
        // can take several seconds from cold. A session shorter than that records
        // nothing at all. See /recording/overview#recording-head-delay.
