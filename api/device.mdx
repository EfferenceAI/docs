...
ERROR_CODE set_ae_settle(bool enabled);   // recording head delay; persisted
ERROR_CODE reboot();
```
`set_ae_settle()` turns the recording head delay on or off, device-wide and
across reboots. See [Recording head delay](/recording/overview#recording-head-delay)
for what it costs and when to turn it off. Returns `UNSUPPORTED` on firmware that
has no head delay; read the value back from `get_device_information().ae_settle`.
