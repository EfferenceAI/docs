...
ERROR_CODE download_recording(
    const std::string& name, const std::string& dest_path,
...
Supply `on_progress` to follow a long pull:
...
It reports the pull once the destination is open: once before any bytes are
written, then as they arrive, on the calling thread. A call that fails before that
never invokes it, and nothing is reported after a failure, so treat the returned
`ERROR_CODE` as the outcome rather than the last progress you saw. Keep it cheap,
do not throw, and do not call back into the `Device` from it.

`resumed_from` is what tells you which bytes were already on disk, so a resumed
pull can report an honest rate.
