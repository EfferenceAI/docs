...
**Contract highlights** (from `Device.hpp`):

- `open()` claims the device **and starts capture** on transports that carry
  a data plane (USB, MCAP; BLE + `udp_host` for WiFi streaming). `grab()`
  works immediately.
...
`UNSUPPORTED_COMPRESSION`), syncs the device clock, and starts capture.
`close()` stops any host recording and releases the transport. `get_state()`
...
```
Decode/convert the grabbed frame into `mat` in the requested `VIEW`. Views
other than `NV12` require the FFmpeg build. The `Mat` owns its buffer.

```cpp
ERROR_CODE retrieve_imu(SensorsData& data,
                        TIME_REFERENCE ref = TIME_REFERENCE::IMAGE);
```
Drain every IMU sample since the previous call. Samples are never lost
between grabs — they queue in the SDK.
...
```
`HOST_FILE` tees the live stream to a local `.mcap`; `DEVICE_LOCAL` records
on the device's eMMC and survives host disconnect. `download_recording` pulls
over the control link (USB/BLE — no WiFi needed); `upload_recording` pushes
from the device to a pre-signed URL over WiFi.
...
```
`url`: `""` = configured server, an `http(s)` URL, or a **local `.eff` path**
(sideloaded over the control link). `update()` blocks through apply and
reboot, reporting progress via the callback.
