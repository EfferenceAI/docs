...
struct DeviceInformation {
    ...
    // Recording head delay currently in force. UNKNOWN means the device reported
    // no policy: either firmware with no head delay at all, or one that could not
    // read its own state just then. Treating it as OFF is wrong in both cases.
    AE_SETTLE ae_settle;             // UNKNOWN | ON | OFF
};
```
