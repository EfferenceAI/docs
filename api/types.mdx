...
An owned image buffer filled by `retrieve_image()`:

| Field | Type | Meaning |
|---|---|---|
| `frame_id` | `uint32_t` | monotonic frame counter |
| `timestamp` | `Timestamp` | capture time, device clock |
| `resolution` | `Resolution` | width × height |
| `type` | `MAT_TYPE` | element layout (`U8_C1/C3/C4`, `NV12`) |
| `view` | `VIEW` | the view it was retrieved as |
| `step` | `int` | bytes per row (first plane) |
| `data` | `std::vector<uint8_t>` | pixel buffer (owned) |
...
struct UpdateStatus {
    bool         active;
    UPDATE_STATE state;              // DOWNLOADING / VERIFYING / READY_TO_APPLY / APPLYING
