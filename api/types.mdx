...
struct DeviceInformation {
    ...
    // Recording head delay currently in force. UNKNOWN is not a synonym for OFF:
    // it means the device reported no policy, which is either firmware with no
    // head delay at all or a device that could not read its own state just then.
