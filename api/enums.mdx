...
| `AE_SETTLE` | `UNKNOWN`, `ON`, `OFF` | recording head delay in force; `UNKNOWN` is not `OFF` -- it means the device reported no policy, either firmware with no head delay at all (<= v00.09.20) or a device that could not read its own state just then |
