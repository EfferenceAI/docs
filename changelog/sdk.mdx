...
<Update label="2026-08-12" description="v00.09.19">
  ...
  **Location**
  ...
  - A device now has no location until one is set, rather than reporting a
    default. The opt-out holds on the recording path too.
  - `location set` is parsed strictly, and the CLI says where a location lands.
  ...
  **Uploads and streaming**
  ...
  - Fixed four stream-reassembly defects, including a holed frame being reported
    as complete.
  ...
  **Recordings**
  ...
  - An interrupted recording is no longer always described as a power loss.

  **Documentation**

  - The CLI reference is split into sections rather than one long page.
