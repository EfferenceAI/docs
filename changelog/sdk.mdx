...
<Update label="2026-08-26" description="v00.09.21">
  ...
  **Segmented recordings**
  ...
  - `record start --segment-duration-s <1..86400>` sets how long each segment
    of a session runs. This option and its wire field were previously spelled
    `--chunk-seconds` and `chunk_seconds`; the old spelling is gone.
  - Downloading a segmented recording pulls the whole session. It arrives as a
    directory of `000000.mcap`, `000001.mcap` and so on, because the segments
    are separate files.
  - A segment can be uploaded as soon as it is sealed, without waiting for the
    session to end.
  ...
  **Fixes**
  ...
  - A chunked download wrote every segment to the same truncated path. Each
    segment now lands at its own file.
  - A stop the device did not confirm is no longer reported as a failed stop.

  **Requirements**

  - The compiler and optional-dependency requirements are stated in the SDK
    README, and the Clang version floor has been dropped.
