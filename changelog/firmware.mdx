...
<Note>
  Version numbers carry two forms: a display string (`00.09.18`) and a monotonic
  integer (`VERSION_INT=918`) used for update comparison and anti-rollback. The
  integer must always increase.
</Note>

<Update label="Unreleased" description="v00.09.19">

  **Recording capacity**

  - The disk-full reserve is now an absolute 512 MiB instead of 10% of the
    recording store. The reserve exists to hold one staged update plus log
    headroom, and that requirement does not grow with the size of the medium.
    On a 12 GB store this returns roughly 700 MB to recordings — about 29 extra
    minutes at 840 Hz IMU, or 43 minutes at 120 Hz.
  - Devices already in the field are migrated automatically on first boot after
    the update. A device that had never been given an explicit reserve adopts
    the new value; an operator-chosen reserve is left untouched.
  ...
  **Mass storage**
  ...
  - The volume follows the USB control-plane lock: a locked device does not
    expose it.

  **Reliability**

  - Fixed a use-after-free that could crash the recorder when a recording failed
    to finalize — reachable whenever the storage write failed, including on a
    full disk.
  - A Wi-Fi lease that nothing renewed could be lost silently; leases are now
    renewed.
  - Bluetooth LE no longer evicts the peer that is currently connected when the
    bond list is full.
  - Session metadata and its directory are now flushed to disk on save, so a
    power cut cannot leave a recording described by a partially written file.
...
<Update label="2026-08-02" description="v00.09.17">
  ...
  <Note>
    This release shipped without notes. The summary below was reconstructed from
    the commit history and has not been reviewed against the original intent.
  </Note>
