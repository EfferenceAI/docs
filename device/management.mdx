...
Stop capture, recording, upload, health, or update work before rebooting. When
the reboot command is accepted—or the link disappears because reboot began
before the reply—the SDK closes the handle. Reconnect with `open()` after the
device returns.
...
```sh
ef config
ef config set 1920 1200 30 h265
ef storage
ef sync-time
ef time
ef location
ef location set 37.7749 -122.4194
ef set-password <new>          # USB
ef --ble <MAC> set-password <old> <new>
ef reboot
```
