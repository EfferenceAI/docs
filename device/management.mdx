...
The empty-old-password form is physical access resetting a forgotten credential,
and it works only while USB is unlocked. Once locked, that shortcut is gone and
`factory_reset()` is the way back in.

The method keeps its `set_ble_password` name for source compatibility, but it has
not been BLE-only since the USB lock shipped. Locking, at-rest encryption, key
handling and recovery are covered in [Access Control and
