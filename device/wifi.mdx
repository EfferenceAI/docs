...
`wifi_state` is one of `connected`, `connecting`, `auth_failed`, or
`disconnected`. `connecting` appears only during a live attempt;
`auth_failed` means the credentials were rejected. A dropped link reads
`disconnected` while the device reconnects on its own.
