...
WiFi status is exposed through the `wireless` field of `DeviceInformation`,
cached when `open()` succeeds. WiFi add/remove/select calls refresh that cached
block after the command is accepted:
...
`wifi_state` is a string carrying the association phase: `connected`,
`connecting`, `disconnected`, or `auth_failed`. Treat `auth_failed` as a
rejected passphrase rather than a transient failure — the device will not retry
its way out of it, so re-add the network with the correct credentials.
Firmware predating this field leaves it empty, in which case the SDK derives
`connected` or `disconnected` from `wifi_connected`.
