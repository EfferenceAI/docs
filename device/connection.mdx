...
An incorrect password does **not** fail the `open()`. The device still answers
`info`, `state` and `storage` unauthenticated, and those are exactly what an
operator needs to diagnose a device whose password they have lost, so failing
here would hide them behind the credential being recovered. The gated verbs
report `INVALID_PASSWORD` on their own. Check up front with
`device.is_authenticated()`, which is meaningful on any BLE link and on a USB one
the device reports as locked. See [Access Control and
...
`IDLE`; existing device activity may report `STREAMING`.
...
`INVALID_PASSWORD` is not in this table on purpose: authentication failure does
not surface from `open()`, only from the gated verb you call afterwards.
