...
How long convergence takes depends on the scene and on the camera tuning, so
measure your own if the margin matters. The device gives up after **7 s by
default** and records anyway; that default is per-device, so treat it as a
setting rather than a hard ceiling. A session comfortably longer than the cap
cannot be swallowed whole, whatever convergence turns out to be.
