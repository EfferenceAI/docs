...
all queued samples even if you never call `retrieve_imu()`. If the process
...
An empty name lets the M1 generate `rec_<utc>_<seq>`. Device-local recording
...
`DISK_FULL` (the device finalizes cleanly when the recording store reaches its
reserve), `WRITE_ERROR`, or `INTERRUPTED` for a power cut. It reads
...
covering everything flushed before the cut. An encrypted one cannot be
repaired on the device (the ciphertext is opaque to it), so it is listed with
