...
By default the device holds the opening frames of a **recording** until
auto-exposure converges, so a file never starts on visibly wrong exposure. The
cost is that the file starts later, and **a session shorter than convergence
records nothing at all** -- neither video nor IMU, because the IMU pre-roll is
discarded along with the video it is aligned to.

Convergence measured on M1 with the sc233hgs:

- **~1.2 s** on a lit bench, the ordinary case.
- **4.4 to 6.1 s** cold in a dark room, the worst seen.
- The device gives up after **7 s by default** and records anyway. That default is
  per-device, so treat it as a setting rather than a hard ceiling.

<Note>
  These figures predate the current camera tuning and have not been re-measured
  against it. Auto-exposure convergence moves with tuning, so do not size a
  capture window against them. The number that holds regardless is the **7 s**
  cap: a session comfortably longer than that cannot be swallowed whole, whatever
  convergence turns out to be. Measure your own scene if the margin matters.
</Note>

If you are recording short clips, either record for longer than convergence or
turn the delay off and accept the exposure ramp in the first frames:

```sh
ef-cli config ae-settle off
```

The setting is device-wide and survives reboot. Live paths -- UVC webcam, ISO
streaming and WiFi streaming -- are never held by it either way. The SDK
equivalent is [`Device::set_ae_settle()`](/api/device).

`ef-cli info --json` reports it as `ae_settle`, with four values. `on` and `off`
are the policy in force. `none` means the firmware has no head delay at all, so it
records from the first frame and the verb returns `UNSUPPORTED`. `unknown` means
the firmware has a policy but did not report it just then; read it again rather
than taking it as `off`.
