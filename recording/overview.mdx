...
are the policy in force. `none` means the firmware has no head delay at all --
it predates the feature, and the verb returns `UNSUPPORTED`. `unknown` means the
firmware has a policy but did not report one just then; re-read it rather than
treating it as `off`.
