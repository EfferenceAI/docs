...
one schema. Recordings also open directly in Foxglove. To extract the raw
...
```
The script writes an elementary stream, not a container. Wrap it to get a
playable file, for example `ffmpeg -i session.h265 -c copy session.mp4`.
